// ParentLogin.tsx - Parent login with password toggle
...
export default function ParentLogin() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      const user = await signInWithGoogle("parent");
      await updateLastLogin(user.uid);
      navigate("/parent-dashboard");
    } catch (err: any) {
  ...
  return (
    <div className="relative min-h-screen w-full bg-gradient-to-br from-orange-400 via-amber-500 to-orange-600 flex flex-col justify-center items-center p-4">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_120%,rgba(251,146,60,0.3),rgba(255,255,255,0))]" />
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-orange-100 text-lg font-medium">Parent Portal</p>
        </div>

        <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl p-8">
          <p className="text-gray-600 text-center mb-6">
            {isLogin ? "Sign in to view your child's progress" : "Create a new parent account"}
          </p>
          ...
          {isLogin ? (
            <form onSubmit={handleEmailLogin} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full !bg-gradient-to-r from-orange-500 to-amber-600 text-white py-4 rounded-xl
          ...
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 !bg-white border-2 border-gray-200 py-3 rounded-xl font-semibold hover:!bg-gray-50 transition-all duration-200 disabled:opacity-50"
          >
